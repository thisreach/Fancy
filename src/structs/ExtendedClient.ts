...
export class ExtendedClient extends Client {
    ...
    private registerCommands(commands: Array<ApplicationCommandDataResolvable>) {
        this.application?.commands
            .set(commands)
            .then(() => {
                console.log('All commands (/) have been updated.'.green);
            })
            .catch(error => {
                console.log(
                    `An error occured white tring to set the Slash commands (/): \n${error}`.red
                );
            });
    }
            ...
            fs.readdirSync(commandsPath + `/${local}/`)
                ...
                .forEach(async fileName => {
                    ...
                    if (name) {
                        ...
                        if (selects)
                            selects.forEach((execute, key) => this.selects.set(key, execute));
    ...
    private registerEvents() {
        ...
        fs.readdirSync(eventsPath).forEach(local => {
            fs.readdirSync(`${eventsPath}/${local}`)
                .filter(fileCondition)
                .forEach(async fileName => {
                    const { name, once, execute }: EventType<keyof ClientEvents> = (
